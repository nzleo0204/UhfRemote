...
import com.leo.uhf.rfid.sdk.storage.*;
import com.leo.uhf.rfid.sdk.bridge.*;
import com.leo.uhf.rfid.sdk.connection.serial.*;
...
public final class ReaderConnectionManagerTest {
    ...
    @Test
    public void connectSerial_delegatesToSessionOrchestrator() {
        AtomicReference<SerialConfig> receivedConfig = new AtomicReference<>();
        AtomicReference<SerialPowerController> receivedPower = new AtomicReference<>();
        manager.setSerialConnector((config, power) -> {
            receivedConfig.set(config);
            receivedPower.set(power);
        });
        SerialConfig config = new SerialConfig("/dev/ttyS1", 115200,
                ModuleSubtype.R2000, 0);
        SerialPowerController power = new DelayPowerController(0);

        manager.connectSerial(config, power);

        assertSame(config, receivedConfig.get());
        assertSame(power, receivedPower.get());
    }
