...
public final class ReaderConfigurationManagerTest {
    ...
    @Test
    public void rm8011QWrite_usesMagicQueryAndStoresFixedQ() {
        assertEquals(0, manager.setQ(ModuleSubtype.RM8011, true, 5, 0, 15, 0, 1));

        assertEquals(5, gateway.lastQ);
        assertFalse(manager.getConfiguration().dynamicQ);
        assertEquals(5, manager.getConfiguration().qValue);
    }
