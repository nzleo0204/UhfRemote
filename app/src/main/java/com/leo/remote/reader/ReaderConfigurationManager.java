...
public final class ReaderConfigurationManager {
    ...
    ReaderConfigurationManager(@NonNull UhfSdkGateway gateway,
    ...
    int setQ(@NonNull ModuleSubtype subtype, boolean dynamic, int qValue, int minQValue,
            int maxQValue, int retryCount, int thresholdMultiplier) {
        ReaderConfiguration current = requireConfiguration();
        int status = subtype == ModuleSubtype.RM8011
                ? gateway.setMagicQuery(current.session, current.target, qValue)
                : gateway.setQ(dynamic, qValue, minQValue, maxQValue, retryCount,
                        thresholdMultiplier, current.qToggleTarget,
                        current.qRepeatUntilNoTags);
        return update(subtype, status, copy(current, current.powerTenthsDbm,
                current.blfProfile, current.session, current.target,
                subtype != ModuleSubtype.RM8011 && dynamic, qValue, minQValue, maxQValue,
                retryCount, thresholdMultiplier, current.inventoryArea,
                current.inventoryAddress, current.inventoryWordLen));
    }
