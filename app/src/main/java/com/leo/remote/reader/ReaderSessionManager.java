...
public final class ReaderSessionManager {
    ...
    public static void initialize(@NonNull Application application) {
        ReaderSessionCoordinator.initialize(application);
    ...
    public static ReaderSessionManager getInstance(@NonNull Application application) {
        if (instance == null) {
            synchronized (ReaderSessionManager.class) {
                if (instance == null) {
                    instance = new ReaderSessionManager(
                            ReaderSessionCoordinator.getInstance(application));
                }
            }
        }
        return instance;
    ...
    static ReaderSessionManager createForTest(Application application, UhfSdkGateway gateway) {
        return new ReaderSessionManager(
                ReaderSessionCoordinator.createForTest(application, gateway));
    }
    ...
    public ReaderTag getCurrentTag() { return coordinator.getCurrentTag(); }
    public int getInventoryMode() { return coordinator.getInventoryMode(); }
    ...
    public String getSavedWifiAddress() { return coordinator.getSavedWifiAddress(); }
    ...
    public void releaseNative() { coordinator.releaseNative(); }
    ...
    public CompletableFuture<Integer> setQ(boolean dynamic, int qValue, int minQValue,
        ...
        return coordinator.setQ(dynamic, qValue, minQValue, maxQValue, retryCount,
                thresholdMultiplier);
    }
    ...
    public boolean hasInventoryMask() { return coordinator.hasInventoryMask(); }
    @Nullable
    ...
    void onConnectionServiceCreated(@NonNull ReaderConnectionService service) {
        coordinator.onConnectionServiceCreated(service);
    }
    void onConnectionServiceDestroyed(@NonNull ReaderConnectionService service) {
        coordinator.onConnectionServiceDestroyed(service);
    }
    void handleUnexpectedDisconnect(@NonNull DisconnectReason reason) {
        coordinator.handleUnexpectedDisconnect(reason);
    }
}
