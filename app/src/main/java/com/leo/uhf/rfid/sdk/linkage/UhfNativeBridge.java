...
public final class UhfNativeBridge implements ReaderTransportGateway,
    ...
    /** 设置 BLE/Wi-Fi 的 RM70XX 板卡透传解析模式，不代表具体射频模块型号。 */
    @Override
    public void useRm70xx() {
        configuredModuleSubtype = null;
        linkage.setRFModuleType(LINKAGE_TYPE_RM70XX);
    }

    /** 设置串口直连模块的射频解析模式，例如 RM8011 旗连对应类型 1。 */
    @Override
    public void setModuleSubtype(ModuleSubtype subtype) {
        int linkageType = linkageTypeFor(subtype);
        configuredModuleSubtype = subtype;
        linkage.setRFModuleType(linkageType);
    }

    /** 返回指定直连模块对应的 Linkage 射频解析类型。 */
    static int linkageTypeFor(ModuleSubtype subtype) {
        if (subtype == null || subtype == ModuleSubtype.UNKNOWN) {
            throw new IllegalArgumentException("串口模块型号无效");
        }
        return subtype.isQilian() ? LINKAGE_TYPE_QILIAN : subtype.getRawValue();
    }

    @Override
    public void setTransport(ConnectionType transport) {
        // Linkage: 0=串口，1=Wi-Fi，2=蓝牙透传。
        int mode = transport == ConnectionType.SERIAL ? 0
                : transport == ConnectionType.WIFI ? 1 : 2;
    ...
    public ReaderConfiguration readConfiguration(ModuleSubtype subtype) throws ReaderException {
        ...
        Integer blf = subtype.isQilian() ? 0 : getBlfProfile();
        ...
        int mode = ReaderConfiguration.defaultsFor(subtype).inventoryMode;
