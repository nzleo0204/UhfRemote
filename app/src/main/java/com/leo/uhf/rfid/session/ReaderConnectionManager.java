...
public final class ReaderConnectionManager {
    ...
    public interface SerialConnector {
        void connect(SerialConfig config, SerialPowerController powerController);
    }
    ...
    private volatile SerialConnector serialConnector;
    ...
    void setSerialConnector(@NonNull SerialConnector connector) {
        serialConnector = connector;
    }

    /** 连接串口读写器，并交由会话编排器执行实际握手。 */
    public void connectSerial(@NonNull SerialConfig config,
            @NonNull SerialPowerController powerController) {
        SerialConnector connector = serialConnector;
        if (connector == null) {
            throw new IllegalStateException("串口连接委托尚未初始化");
        }
        connector.connect(config, powerController);
    }
