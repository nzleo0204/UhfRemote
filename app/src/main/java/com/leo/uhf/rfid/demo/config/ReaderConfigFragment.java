...
import com.leo.uhf.rfid.api.model.DisconnectReason;
import com.leo.uhf.rfid.api.model.InventoryArea;
import com.leo.uhf.rfid.api.model.Rm610PowerLevels;
import com.leo.uhf.rfid.api.model.ModuleSubtype;
import com.leo.uhf.rfid.api.model.ReaderConfiguration;
import com.leo.uhf.rfid.api.model.ReaderConnectionStatus;
import com.leo.uhf.rfid.api.model.ReaderException;
...
import com.leo.uhf.rfid.api.model.ConnectionType;
import com.leo.uhf.rfid.transport.serial.SerialConfig;
import com.leo.uhf.rfid.demo.connection.BleDeviceSheet;
import com.leo.uhf.rfid.demo.config.DeviceInfoDialog;
import com.leo.uhf.core.ui.dialog.MessageDialog;
import com.leo.uhf.core.ui.dialog.SelectDialog;
import com.leo.uhf.rfid.demo.config.InventoryRangeDialog;
import com.leo.uhf.rfid.demo.config.ReaderConnectionDialogController;
import com.leo.uhf.rfid.demo.config.ReaderSettingDialogController;
import com.leo.uhf.rfid.demo.connection.SerialConfigDialog;
import com.leo.uhf.rfid.demo.common.ReaderFragment;
import com.leo.uhf.core.ui.widget.IpAddressInputView;
import com.leo.uhf.core.util.ThrowableUtils;
import com.leo.uhf.core.util.ViewUtils;
...
public final class ReaderConfigFragment extends ReaderFragment implements ReaderObserver {
    ...
    protected void initView() {
        ...
        bindingUi = false;
        bindTransportRows(ConnectionType.BLE, false);
        ...
        bleSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            bindingUi = false;
            dismissWifiKeyboard();
            bindTransportRows(ConnectionType.BLE, readerState.hasTransportLink());
        });
        wifiSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            serialSwitch.setChecked(false);
            bindingUi = false;
            bindTransportRows(ConnectionType.WIFI, readerState.hasTransportLink());
        });
        serialSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            bindingUi = false;
            dismissWifiKeyboard();
            bindTransportRows(ConnectionType.SERIAL, readerState.hasTransportLink());
        });
    ...
    public void onReaderStateChanged(ReaderState state) {
        ...
        if (state.getTransport() == ConnectionType.BLE) {
            bindingUi = true;
            bleSwitch.setChecked(true);
            wifiSwitch.setChecked(false);
            bindingUi = false;
        } else if (state.getTransport() == ConnectionType.WIFI) {
            bindingUi = true;
            bleSwitch.setChecked(false);
            wifiSwitch.setChecked(true);
            bindingUi = false;
        } else if (state.getTransport() == ConnectionType.SERIAL) {
            bindingUi = true;
            bleSwitch.setChecked(false);
            wifiSwitch.setChecked(false);
        ...
        ConnectionType selected = state.getTransport() == ConnectionType.NONE
                ? (bleSwitch.isChecked() ? ConnectionType.BLE
                : wifiSwitch.isChecked() ? ConnectionType.WIFI : ConnectionType.SERIAL)
                : state.getTransport();
        bindTransportRows(selected, state.hasTransportLink());
        setHardwareEnabled(connected);
        if (state.getTransport() == ConnectionType.BLE && !state.getAddress().isEmpty()) {
    ...
    private void bindTransportRows(ConnectionType transport, boolean connected) {
        bleActions.setVisibility(!connected && transport == ConnectionType.BLE ? View.VISIBLE : View.GONE);
        wifiActions.setVisibility(!connected && transport == ConnectionType.WIFI ? View.VISIBLE : View.GONE);
        serialActions.setVisibility(!connected && transport == ConnectionType.SERIAL ? View.VISIBLE : View.GONE);
        ...
        if (connected) {
            String target = readerState.getTransport() == ConnectionType.BLE
    ...
    private void showDeviceInfo() {
        ...
        if (getParentFragmentManager().findFragmentByTag(DEVICE_INFO_DIALOG_TAG) == null) {
            new DeviceInfoDialog().show(getParentFragmentManager(), DEVICE_INFO_DIALOG_TAG);
        }
    }
    ...
    private void applyModuleUi(ModuleSubtype subtype) {
        boolean isRm8011 = subtype.isQilian();
        boolean isRm610Discrete = subtype == ModuleSubtype.RM610
                && !Rm610PowerLevels.isCmtVersion(readerState.getModuleSerial());
        boolean connected = readerState.isConnected();
        int maxDbm = subtype.isQilian() || subtype == ModuleSubtype.RM610 ? 20 : 30;
