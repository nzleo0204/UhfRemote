...
- 蓝牙（BLE）
- Wi-Fi
- 串口直连（R2000、R2000Plus、RM610、RM8011 旗连/MagicRF）
...
串口设备节点权限由客户设备平台控制，可能需要系统签名、设备用户组或 root 配置。当前 SDK 的 `Linkage` 已提供串口打开/关闭 API，因此生产连接不额外复制未经验证的 JNI 库；如客户平台需要不同的上电方式，实现 `SerialPowerController` 即可接入，串口协议仍由 Linkage 负责。
